...
class NodesHandler {
  ...
  refresh(clearPositions = false) {
    ...
    for (let nodeId in nodes) {
      ...
      let data = this.body.data.nodes.get(nodeId); //OB change _data[nodeId] by get(nodeId)
  ...
  storePositions() {
    ...
    for (let nodeId in dataset._data) {
      if (dataset._data.hasOwnProperty(nodeId)) {
        ...
        if (node && (dataset._data[nodeId].x != Math.round(node.x) || dataset._data[nodeId].y != Math.round(node.y))) { //OB Add test if node exist (See #903) . Utile ??? (See. #903)
